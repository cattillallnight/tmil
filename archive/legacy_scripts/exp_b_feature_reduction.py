...
from src.model import GatedTMILETH, GatedCompoundLoss
from src.train import AccountWindowDataset, collate_fn, train_one_epoch, evaluate_epoch

import json
from collections import defaultdict
import pandas as pd
from src.utils import PHISHER_TX_IN, PHISHER_TX_OUT
from src.evaluate import load_transactions
...
def evaluate_on_tornado(model, records, W, S):
    ...
    for addr, gt_hashes in tc_gt.items():
        ...
        for win_idx, (start, end) in enumerate(wins):
            ...
            # EXPERIMENT B: Feature Reduction (Keep z_amount and value_ratio)
            # hand_crafted features are [z_amount, density, counterparty_novelty, value_ratio]
            # So we keep indices 0 and 3
            hc_win_reduced = hc_win_pad[:, [0, 3]]

            hc_t = torch.tensor(hc_win_reduced, dtype=torch.float32).unsqueeze(0).to(DEVICE)
...
# Custom collate fn for Experiment B to reduce features on the fly
def collate_fn_reduced(batch):
    ...
    return hc_reduced, bert_bcast, labels

def main():
    W = 200
    S = 50
    print(f"--- Running Experiment B: Feature Reduction (66 dims) for W={W}, S={S} ---", flush=True)
    ...
    train_loader = DataLoader(train_ds, batch_size=32, sampler=sampler, collate_fn=collate_fn_reduced, num_workers=0)
    val_loader   = DataLoader(val_ds,   batch_size=32, shuffle=False, collate_fn=collate_fn_reduced, num_workers=0)

    # EXPERIMENT B: hand_crafted_dim is now 2
    model = GatedTMILETH(hand_crafted_dim=2, bert_dim=64, proj_dim=64, attn_hidden=128, mlp_hidden=256).to(DEVICE)
    # EXPERIMENT B: keep original consistency loss (lambda1=0.3)
