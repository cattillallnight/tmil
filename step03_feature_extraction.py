...
def build_account_dataset(accounts: list, labels: dict,
                          ...
                          tag: str, vocab: dict, max_rows: int = None) -> list:
    """
    Build per-account feature records.
    Returns list of dicts with keys: address, label, windows, n_tx, counterparty_ids
    """
    ...
    for addr in tqdm(accounts, desc=f"  Features [{tag}]"):
        ...
        cp_ids = np.array([vocab.get(tx[3], 0) for tx in tx_list], dtype=np.int32)
        is_out = np.array([tx[2] == "OUT" for tx in tx_list], dtype=bool)
        ...
        records.append({
            ...
            "counterparty_ids": cp_ids,    # (n_tx,)
            "is_outbound": is_out,         # (n_tx,)
...
def main():
    ...
    # Load Counterparty Vocab
    print("\nLoading counterparty vocabulary...")
    vocab_file = RESULTS_DIR / "counterparty_vocab.pkl"
    if not vocab_file.exists():
        print("ERROR: Run step15_build_counterparty_vocab.py first!")
        sys.exit(1)
